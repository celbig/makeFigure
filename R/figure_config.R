parse_figure_config = function(config_raw) {
  ...
  raw_dir = pluck(global, "raw_output_dir", .default = file.path(output_dir, "raw"))
  individual_plot_dir = pluck(global, "individual_plot_dir", .default = file.path(script_dir, "plots"))
  figures = compact(imap(config_raw, parse_figure_config_field, global = global))
  if (is_empty(figures)) {
  ...
  try_or_error({
    ...
    if(is.character(height) && (height %in% accepted_height_values)) {
      if (is(global[["height"]][[height]], "numeric")) {
        height = is(global[["height"]][[height]])
      } else {
        abort(glue("Configuration error: {.field .global} > {.global height} > {.field {height}} must be a numeric value"))
      }
    }
    ...
    if(is.character(width) && (width %in% accepted_width_values)) {
      if (is(global[["width"]][[width]], "numeric")) {
        height = is(global[["width"]][[width]])
      } else {
        abort(glue("Configuration error: {.field .global} > {.field width} > {.field {height}} must be a numeric value"))
      }
    }
    ...
    individual_plots = pluck(field_config, "individual_plots", .default = character())
    if(!is.character(individual_plots)) {
      individual_plots = as.character(individual_plots)
    }
    if (is_empty(individual_plots)){
      abort("No individual plot defined!")
    }
    
    used_keys = c("height", "width", "ratio", "script", "data", "params", "library", "individual_plots")
